...
var WHHoje = new Date();
var WHData = `${WHHoje.getDate()}/${WHHoje.getMonth() +
  1}/${WHHoje.getFullYear()}`;
...
client.on(`guildMemberAdd`, member => {
  ...
  member.send(
    new Discord.RichEmbed()
      ...
      .setDescription(
        `**Olá, eu sou o HunterHelper, um bot criado pelo RCPOLSKI especificamente para o TheHunter Classic**`
      )
      .setColor(`#2DCC70`)
      .addBlankField()
      .addField(
        `Irei lhe mostrar alguns comandos iniciais para você usar o servidor ao máximo!`
      )
      .addBlankField()
      ...
      .setFooter(
        `Qualquer dúvida, pode chamar o RCPOLSKI (RCPOLSKI#1827) no servidor`
      )
  );
  ...
  webhook.send(
    new Discord.RichEmbed()
      .setTitle(
        `${member.user.username} (${member.user.id}) > Entrou no servidor`
      )
      .setTimestamp()
  );
});
client.on(`guildMemberRemove`, member => {
  ...
  webhook.send(
    new Discord.RichEmbed()
      .setTitle(
        `${member.user.username} (${member.user.id}) > Saiu do servidor`
      )
      .setTimestamp()
  );
});
client.on("presenceUpdate", (oldMember, newMember) => {
  if (
    oldMember.user.presence.status ||
    newMember.user.presence.status === `online`
  ) {
  } //Test
  ...
  if (fs.existsSync(`./perfis/registrados/${newMember.user.id}.json`)) {
    var JSONCheck = new jSelfReloadJSON(
      `./perfis/registrados/${newMember.user.id}.json`
    );
    JSONCheck.resume();
    ...
    if (Horas != Hour) {
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${newMember.user.username} (${newMember.user.id}) > Iniciando atualização de perfil (★${pView.HUNTERSCORE})`
          )
          .setTimestamp()
      );
      console.log(
        `Hora de atualizar o perfil de ${newMember.user.username}(${newMember.user.id})`
      );
      JSONCheck.stop();
      updatePlayerInfo(newMember.user.id);
    }
  }
  ...
  function updatePlayerInfo(id) {
    ...
    var no_role = newMember.guild.roles.find(
      a => a.name === `Caçador sem licença`
    );
    var in_role = newMember.guild.roles.find(
      b => b.name === `Nivel Iniciante 0 - 300`
    );
    var az_role = newMember.guild.roles.find(
      c => c.name === `Nivel Aprendiz  300+`
    );
    var am_role = newMember.guild.roles.find(
      d => d.name === `Nivel Amador 900+`
    );
    var cd_role = newMember.guild.roles.find(
      e => e.name === `Nivel Caçador 1800+`
    );
    var it_role = newMember.guild.roles.find(
      f => f.name === `Nivel Instrutor 3000+`
    );
    var gu_role = newMember.guild.roles.find(
      g => g.name === `Nivel Guia 5000+`
    );
    var ep_role = newMember.guild.roles.find(
      h => h.name === `Nivel Épico 7500+`
    );
    var ld_role = newMember.guild.roles.find(
      i => i.name === `Nível Lendário 10.000+`
    );
    var mt_role = newMember.guild.roles.find(
      j => j.name === `Nível Mítico 25.000+`
    );
    var ex_role = newMember.guild.roles.find(
      k => k.name === `Nivel Extraordinário 50.000+`
    );
    var ec_role = newMember.guild.roles.find(
      l => l.name === `Nivel Embaixador de Caça 100.000+`
    );
    var autorID = newMember.user.id; // ID do usuário
    var p = autorID;
        ...
        try {
          ...
          try {
            ...
            webhook.send(
              new Discord.RichEmbed()
                .setTitle(
                  `${newMember.user.username} (${newMember.user.id}) > Erro ao atualizar perfil`
                )
                .setTimestamp()
            );
            console.log(err);
          ...
          try {
            switch (numberRole) {
              ...
              default:
                webhook.send(
                  new Discord.RichEmbed()
                    .setTitle(
                      `${member.user.username} (${member.user.id}) > Não foi possível atualizar o perfil`
                    )
                    .setTimestamp()
                );
            ...
            webhook.send(
              new Discord.RichEmbed()
                .setTitle(
                  `${member.user.username} (${member.user.id}) > Erro ao atualizar perfil => ${error}`
                )
                .setTimestamp()
            );
            console.error(error);
          ...
          function levelUp() {
            ...
            webhook.send(
              new Discord.RichEmbed()
                .setTitle(
                  `${member.user.username} (${member.user.id}) > Upou para o level ${myRole}`
                )
                .setTimestamp()
            );
          }
          ...
          webhook.send(
            new Discord.RichEmbed()
              .setTitle(
                `${newMember.user.username} (${newMember.user.id}) > Teve seu perfil atualizado (★${pView.HUNTERSCORE})`
              )
              .setTimestamp()
          );
        } catch (error) {
          webhook.send(
            new Discord.RichEmbed()
              .setTitle(
                `${newMember.user.username} (${newMember.user.id}) > Erro ao atualizar o registro => ${error}`
              )
              .setTimestamp()
          );
          console.log(
            `Ops, não consegui atualizar o registro de ${newMember.user.username}`
          );
          console.log(Error);
          return;
        }
...
client.on(`message`, message => {
  ...
  if (fs.existsSync(`./perfis/registrados/${message.author.id}.json`)) {
    ...
    if (Horas != Hour) {
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Inicializando atualização de perfil (★${pView.HUNTERSCORE})`
          )
          .setTimestamp()
      );
      console.log(
        `Hora de atualizar o perfil de ${message.author.username}(${message.author.id})`
      );
      JSONCheck.stop();
  ...
  var matchCommand = elCommand.match(
    /\bcomandos|\breservas|\bsobre|\bid|\bme|\bp|\br|\bm/g
  );
  if (
    message.content.startsWith(prefix) &&
    !matchCommand &&
  ...
  if (message.content.startsWith(prefix) && isOK === 1) {
    ...
    if (comando === `idle` && message.author.username === `RCPOLSKI`) {
      ...
      webhook.send(
        new Discord.RichEmbed().addField(`Entrei em modo de atualização`)
      );
    ...
    if (comando === `!` && autorUsername === `RCPOLSKI`) {
      ...
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `Essas são a diretrizes propostas para uma comunidade saudável!`
          )
          .setColor(`#F1C40F`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          .setTitle(`**1.** SPAM`)
          .setDescription(
            `Não faça spam nos chats com mensagens, fragmentos de mensagens ou publique grandes parágrafos de texto.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          .setTitle(`**2.** OFENSAS`)
          .setDescription(
            `Não pratique discursos de ódio, insultos raciais ou qualquer tipo de pensamento ofensivo. Não importa sua raça ou gênero... Este comportamento NÃO será tolerado.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `Trate os outros caçadores como você gostaria de ser tratado. Não intimide, assedie ou seja tóxico para com outros usuários.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `Não se junte ao chat de texto, voz ou no jogo apenas para insultar o caçador, sabemos que você tem sua própria opinião, mas a mesma não é justificativa para um comportamento tóxico, guarde essas mesmas opiniões para si. Crítica construtiva com o consentimento do outro é permitido.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `Nenhum tipo publicidade, em qualquer chat ou DM, a menos que seja verificada e aprovada por um ADM.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
      ...
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `Evite palavrões ou linguagem inapropriada em chat de voz.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `Respeite a todos. Se você estiver incomodado com um membro, envie uma mensagem direta aos ADMs/Moderadores.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          ...
          .setDescription(
            `O envio de fotos/imagens devem ser feitos no ${fChannel}.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
        new Discord.RichEmbed()
          .setTitle(`**10.** MEMES`)
          .setDescription(
            `Envio de memes, vídeos engraçados ou qualquer mídia que possa ser **MEME** devem ser enviadas em ${memeChannel}.`
          )
          .setColor(`#FFFFFF`)
      );
      message.channel.send(
    ...
    if (comando === `admr` && autorUsername !== `RCPOLSKI`) {
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${rMember.user.username} (${rMember.user.id}) > Foi cadastrado pelo administrador`
          )
          .setTimestamp()
      );
      isAdmR = 1;
    ...
    if (comando === `delr` && autorUsername !== `RCPOLSKI`) {
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${dMember.user.username} (${dMember.user.id}) > Foi cadastrado pelo administrador`
          )
          .setTimestamp()
      );
      delPlayerInfo();
    ...
    if (
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Foi cadastrado pelo administrador`
          )
          .setTimestamp()
      );
      console.log(
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Requisitou informações sobre o comando !r`
          )
          .setTimestamp()
      );
      console.log(
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Iniciou o processo de cadastramento`
          )
          .setTimestamp()
      );
      registerUser();
    ...
    if (comando === `id` && !args.length) {
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Requisitou sua ID`
          )
          .setTimestamp()
      );
      updatePlayerInfo(message.author.id);
        ...
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${
              message.author.id
            }) > Pesquisou pelo nickname de ${args[0]}`
          )
          .setTimestamp()
      );
      profileSearch();
    ...
    if (comando === `m` && args[0] === `?`) {
      ...
      try {
        ...
        webhook.send(
          new Discord.RichEmbed()
            .setTitle(
              `${message.author.username} (${message.author.id}) > Requisitou as informações de ${rData.Nome}`
            )
            .setTimestamp()
        );
        console.log(
    ...
    if (comando === `reservas`) {
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Requisitou as informações das reservas`
          )
          .setTimestamp()
      );
      console.log(
    ...
    if (comando === `sobre`) {
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Requisitou as informações do BOT`
          )
          .setTimestamp()
      );
      console.log(
    ...
    if (comando === `comandos`) {
      ...
      webhook.send(
        new Discord.RichEmbed()
          .setTitle(
            `${message.author.username} (${message.author.id}) > Requisitou as informações de comandos`
          )
          .setTimestamp()
      );
      console.log(
    ...
    if (comando === `update` && autorUsername === `RCPOLSKI`) {
      ...
      message.channel.send(
        new Discord.RichEmbed()
          .setTitle(`Hotfix 1.2g`)
          .setThumbnail(client.user.avatarURL)
          .setColor(`#F1C40F`)
          .setDescription(`Esta atualização contém melhorias`)
          .addBlankField()
          .addField(
            `**Melhorias**`,
            `- O sistema de 1 atualização por dia fora trocada por 1 vez a cada 3 horas\n- Novo sistema de ProfileScrapping\n- Adicionado ProfileFetching\n- Adicionado ProfileExecuteOnTime\n- Adicionado ProfileReadOnTime\n- Adicionado ProfileWriteOnTime`
          )
          .addBlankField()
          .setFooter(
    ...
    function profileSearch() {
      ...
      try {
        (async () => {
          ...
          var THPrecision = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(13) > tbody > tr > td:nth-child(2) > span.small`).text();
          var THLongShot = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(25) > tbody > tr:nth-child(1) > td:nth-child(2) > span.main > strong`).text();
          var THRank = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(2) > td > table:nth-child(3) > tbody > tr > td:nth-child(5) > span.heading`).text();
          var THCss = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(2) > td > table:nth-child(3) > tbody > tr > td:nth-child(7) > span.heading`).text();
          var THAvatar = $(`#polaroid_div > table > tbody > tr > td > table > tbody > tr:nth-child(1) > td:nth-child(2) > img`).attr(`src`);
          var THHunterScoreString = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(2) > td > table:nth-child(3) > tbody > tr > td:nth-child(3) > span.heading`).text();
          var THMinShot = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(25) > tbody > tr:nth-child(1) > td:nth-child(4) > strong`).text();
          var THHarvested = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(37) > tbody > tr > td:nth-child(2) > span.main > strong`).text();
          var THSpotted = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(37) > tbody > tr > td:nth-child(6) > span.main > strong`).text();
          var THShots = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(13) > tbody > tr > td:nth-child(2) > span.main > strong`).text();
          var THTracked = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(37) > tbody > tr > td:nth-child(4) > span.main > strong`).text();
          var THTime = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(10) > tbody > tr > td:nth-child(2) > span.main > strong`).text();
          var THWalk = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(10) > tbody > tr > td:nth-child(4) > span.main > strong`).text();
          var THMushroom = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(13) > tbody > tr > td:nth-child(4) > span.main > strong`).text();
          var THFriend = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(16) > tbody > tr > td:nth-child(4) > span.main > strong`).text();
          var THTrophy = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(4) > td > table:nth-child(16) > tbody > tr > td:nth-child(2) > span.main > strong`).text();
          var THNickname = $(`#user-profile-url`).text();
          var THHunterScore = parseInt(THHunterScoreString);
          if (
            THHunterScore <= -1 ||
            THHunterScore === undefined ||
            THHunterScore === NaN
          ) {
            console.log(
              `[${DataTempo}] ${
            ...
            /* Modelo Antigo
						...
						.addField(`${ms_emoji}  **${Mushroom[3]}**kg`, `Cogumelos coletados`, true)
            ...
            (async () => {
              cChannel.send(
                new Discord.RichEmbed()
                  .setTitle(
                    `O novo perfil está em beta... Portanto, podem acontecer alguns erros!`
                  )
                  .setDescription(
                    `Para relatar algum erro, contate <@261850586008387584>`
                  )
              );
            ...
            async function profile() {
              try {
                ...
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-Black.ttf")
                  ),
                  "Black"
                );
                Canvas.registerFont(
                ...
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-ExtraBold.ttf")
                  ),
                  "ExtraBold"
                );
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-Medium.ttf")
                  ),
                  "Medium"
                );
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-Regular.ttf")
                  ),
                  "Regular"
                );
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-Thin.ttf")
                  ),
                  "Thin"
                );
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-Light.ttf")
                  ),
                  "Light"
                );
                Canvas.registerFont(
                  resolve(
                    join(__dirname, "./assets/fonts/Montserrat-ExtraLight.ttf")
                  ),
                  "ExtraLight"
                );
