...
client.on(`ready`, () => {
	...
	webhook.send(new Discord.RichEmbed()
		.setTitle(`O HunterHelper foi iniciado...`)
		.setTimestamp()
	)
});
...
client.on(`guildMemberAdd`, member => {
	...
	webhook.send(new Discord.RichEmbed()
		.setTitle(`${member.user.username} (${member.user.id}) > Entrou no servidor The Hunter -BR-`)
		.setTimestamp()
	)
...
client.on(`guildMemberRemove`, member => {
	...
	webhook.send(new Discord.RichEmbed()
		.setTitle(`${member.user.username} (${member.user.id}) > Saiu do servidor The Hunter -BR-`)
		.setTimestamp()
	)
...
client.on('presenceUpdate', (oldMember, newMember) => {
	...
	if (fs.existsSync(`./perfis/registrados/${newMember.user.id}.json`)) {
		...
		if (Horas != Hour) {
			webhook.send(new Discord.RichEmbed()
				.setTitle(`${newMember.user.username} (${newMember.user.id}) > Atualizando perfil`)
				.setTimestamp()
			)
		...
		(async() => {
			try {
				...
				try {
					...
					webhook.send(new Discord.RichEmbed()
						.setTitle(`${member.user.username} (${member.user.id}) > Erro ao atualizar perfil`)
						.setTimestamp()
					)
					console.error(error);
				}
				...
				function levelUp() {
					...
					webhook.send(new Discord.RichEmbed()
						.setTitle(`${member.user.username} (${member.user.id}) > Upou para o level ${memberRole}`)
						.setTimestamp()
					)
				}
				...
				webhook.send(new Discord.RichEmbed()
					.setTitle(`${newMember.user.username} (${newMember.user.id}) > Teve seu perfil atualizado`)
					.setTimestamp()
				)
			} catch (Error) {
				webhook.send(new Discord.RichEmbed()
					.setTitle(`${newMember.user.username} (${newMember.user.id}) > Erro ao atualizar o registro`)
					.setTimestamp()
				)
				console.log(
...
client.on(`message`, message => {
	...
	if (fs.existsSync(`./perfis/registrados/${message.author.id}.json`)) {
		...
		if (Horas != Hour) {
			webhook.send(new Discord.RichEmbed()
				.setTitle(`${message.author.username} (${message.author.id}) > Atualizando o perfil`)
				.setTimestamp()
			)
	...
	if (message.content.startsWith(prefix) && isOK === 1) {
		...
		if (comando === `idle` && message.author.username === `RCPOLSKI`) {
			...
			webbhook.send(new Discord.RichEmbed()
		...
		if (comando === `!` && autorUsername === `RCPOLSKI`) {
			...
			try {
				//var ADM_role = message.guild.roles.find(r => r.name === `HunterHelper`);
				//ADM_role.setColor(`#FFFFFF`)
			} catch (err) {}
			message.delete(10);
			return;
		}
		...
		if (comando === `update` && autorUsername === `RCPOLSKI`) {
			...
			message.channel.send(
				...
				.setColor(`#F1C40F`)
				.setDescription(
					`Esta atualização contém melhorias`
				)
				.addBlankField()
				.addField(
					`**Melhorias**`,
...
client.login('NjYyMTMzMzAxMzg0ODM5MTk5.Xg1iPw.YSBTQcQDNo1IJXAgAQUIduxUgkI');
