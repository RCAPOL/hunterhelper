...
const commands = {
        'tocar': (message) => {
            ...
            if (!message.guild.voiceConnection) return commands.aqui(message).then(() => commands.tocar(message))
            ...
            (function play(song) {
                ...
                if (song === undefined) {
                    return message.channel.send(new Discord.RichEmbed()
                        .setColor(`#E84C3D`)
                        .setTitle(`Não há músicas na fila...`)
                    ).then(() => {
                ...
                collector.on(`message`, m => {
                    if (m.content.startsWith(`${tokens.prefix}pausar`)) {
                        message.channel.send(`Pausado`).then(() => {
                            dispatcher.pause()
                        })
                    } else if (m.content.startsWith(`${tokens.prefix}resumir`)) {
                        message.channel.send(`Resumindo`).then(() => {
                            dispatcher.resume()
                        })
                    } else if (m.content.startsWith(`${tokens.prefix}pular`)) {
                        message.channel.send(`Pulado`).then(() => {
                            dispatcher.end()
                        })
                ...
                dispatcher.on(`error`, (err) => {
                    return message.channel.semd(`error: ${err}`).then(() => {
                        collector.stop()
                        play(queue[message.guild.id].songs.shift())
                    })
                })
        ...
        'aqui': (message) => {
            return new Promise((resolve, reject) => {
                ...
                if (!voiceChannel || voiceChannel.type !== `voice`) return message.reply(`Não posso me conectar à sua sala...`)
        ...
        'add': (message) => {
            ...
            yt.getInfo(url, (err, info) => {
                ...
                'fila': (message) => {
                    if (queue[message.guild.id] === undefined) return message.channel.send(`Adicione músicas à fila com o comando ${tokens.prefix}add`)
                    var tosend = []
        ...
        client.on(`guildMemberAdd`, member => {
            ...
            var rt_role = member.guild.roles.find(r => r.name === `Recrutador`)
        ...
        client.on(`message`, message => {
            ...
            if (!fs.existsSync(`./perfis/registrados/${message.author.id}.json`)) {
                message.member.setRoles([no_role])
                    .catch(console.error)
            }
            ...
            var testeChannel = message.guild.channels.find(ch => ch.name === `testebot`)
...
/* 
        ...
        if (comando === `idle` && message.author.username === `RCPOLSKI`) {
            ...
            if (!args.length) {aDescription = `Apenas algumas correções...`}
            if (args[0] === `patch`) {aDescription = `Patch sob demanda! Parece que me deram super poderes... Confira em ${logChannel}`}
            if (args[0] === `update`) {aDescription = `Conteúdo novo área! Confira os detalhes em ${logChannel}`}
            message.delete(100)
            if (!fs.existsSync(`./status/idling.txt`)) {return}
            cChannel.send(new Discord.RichEmbed()
        ...
        if (fs.existsSync(`./status/idling.txt`) && message.author.username !== `RCPOLSKI`) {
            ...
            return
                ...
                if (comando === `!` && autorUsername === `RCPOLSKI`) {
                    var no_role = message.guild.roles.find(a => a.name === `Caçador sem licença`)
                    var in_role = message.guild.roles.find(b => b.name === `Nivel Iniciante 0 - 300`)
                    var az_role = message.guild.roles.find(c => c.name === `Nivel Aprendiz  300+`)
                    var am_role = message.guild.roles.find(d => d.name === `Nivel Amador 900+`)
                    var cd_role = message.guild.roles.find(e => e.name === `Nivel Caçador 1800+`)
                    var it_role = message.guild.roles.find(f => f.name === `Nivel Instrutor 3000+`)
                    ...
                    var ld_role = message.guild.roles.find(i => i.name === `Nível Lendário 10.000+`)
                    var mt_role = message.guild.roles.find(j => j.name === `Nível Mítico 25.000+`)
                    var ex_role = message.guild.roles.find(k => k.name === `Nivel Extraordinário 50.000+`)
                    var ec_role = message.guild.roles.find(l => l.name === `Nivel Embaixador de Caça 100.000+`)
                   ...
                   /*  message.channel.send(new Discord.RichEmbed()    
                    ...
                    try {
                    (async () => {
                    var PlayerLink = `https://www.uhcapps.co.uk/stats_lifetime.php?username=${args[0]}` // Source do Scrapper
                    ...
                    } else {console.log("remainder is lower than 0.5")}
                    ...
                    try {
                        (async () => {
                            ...
                            if (THHunterScoreString === ``) {
                                ...
                                var Precision = 100 - CalcPrecision
                                if((Precision % 1) > 0.5) {Precision + 1}
                                ...
                                /* if (fs.existsSync(`./perfis/pesquisados/${ign[0]}.json`)) {
                                    var pSearch = require(`./perfis/pesquisados/${ign[0]}.json`)
                                    var ProgressMsg = ` ->`
                                    if (THHunterScore === pSearch.HUNTERSCORE) {
                                        ProgressMsg = `   |   -- representa que não houve progresso neste perfil`
                                        pProgHS = ` --`
                                        pProgRK = ` -- `
                                        return (pProgHS, pProgRK, ProgressMsg)
                                    } else if (THHunterScore > pSearch.HUNTERSCORE) {
                                        ProgressMsg = `   |   (+) representa o progresso do jogador`
                                        var HSup = THHunterScore - pSearch.HUNTERSCORE
                                        var RKup = THRank - pSearch.RANK
                                        var pProgHS = ` (+` + HSup + `)`
                                        var pProgRK = ` (+` + RKup + `)`
                                        return (pProgHS, pProgRK, ProgressMsg)
                                    }
                                } else if (!fs.existsSync(`./perfis/pesquisados/${ign[0]}.json`)) {
                                    ProgressMsg = `   |   ^^ representa minha primeira vez mostrando esse perfil`
                                    pProgHS = ` ^^`
                                    pProgRK = ` ^^`
                                    return (pProgHS, pProgRK, ProgressMsg)
                                } */
            ...
            function registerUser() {
                ...
                var ep_role = message.guild.roles.find(h => h.name === `Nivel Épico 7500+`)
                var ld_role = message.guild.roles.find(i => i.name === `Nível Lendário 10.000+`)
                var mt_role = message.guild.roles.find(j => j.name === `Nivel Mítico 25.000+`)
                var ex_role = message.guild.roles.find(k => k.name === `Nivel Extraordinário 50.000+`)
                var ec_role = message.guild.roles.find(l => l.name === `Nivel Embaixador de Caça 100.000+`)
                ...
                if (fs.existsSync(`./perfis/registrados/${p}.json`) && alreadyReg === 1) {
                    ...
                    (async () => {
                        try {
                            ...
                            const response = await rp(PlayerLink)
                            const $ = jCheerio.load(response)
                            var THHunterScore = $(`body > table > tbody > tr > td > table:nth-child(2) > tbody > tr:nth-child(2) > td > table:nth-child(3) > tbody > tr > td:nth-child(3) > span.heading`).text()
                            var THNickname = $(`#user-profile-url`).text()
                            var THAvatar = $(`#polaroid_div > table > tbody > tr > td > table > tbody > tr:nth-child(1) > td:nth-child(2) > img`).attr(`src`)
                            var ign = THNickname.match(/[a-zA-Z,.-_]+|[0-9.,-_]+(?:\.[0-9.,-_]+|)/g)
